...
class StockTests {
    ...
    */
    ...
    void testUpdate() {
        ...
        String stockName = "Update stock";
        String cityName = "Update city";
        Long id = createStock(stockName, cityName);
        //создаем дто с сущностью
        StockRequestDto stockRequestDto = createStockRequestDto(stockName, cityName);

        // body(id) - берём сущность по полученному id.
        RequestEntity<StockRequestDto> requestEntityUpd =  // body(stockRequestDto) - берём сущность по полученному id.
                RequestEntity.post(URI.create(resourceUrl)).contentType(MediaType.APPLICATION_JSON).body(stockRequestDto);

        //выполнение метода /put
        testRestTemplate.exchange(resourceUrl + id, HttpMethod.PUT, requestEntityUpd, STOCK_RESPONSE);
